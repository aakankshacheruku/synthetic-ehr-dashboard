...
How do treatment costs and pathways progress for breast cancer patients, from diagnosis through medication, surgery, radiation, or chemotherapy?

*I originally wanted to compare a first diagnosis against a recurrence, but confirmed directly in the data that Synthea's breast cancer module models each patient's diagnosis as a single event. Rather than forcing a comparison the data couldn't support, I noted this as a limitation of synthetic data: it can model a condition's typical pathway, but not the complex journey of living with the disease twice.*


## How I Approached This

I didn't want to follow a tutorial and end up with something I couldn't explain. For every skill this project required, I studied the basics in short, focused lessons, then immediately applied each lesson to a real piece of the project. That meant writing my own SQL queries and understanding why each step works, like how a JOIN connects two tables.


## Tech Stack

- **Synthea:** Generates realistic synthetic patient data with no privacy concerns
- **PostgreSQL and SQL:** Store and query the data to answer specific questions
- **Tableau Public:** Turns query results into an interactive dashboard
- **GitHub:** Hosts the project publicly, including code, notes, and progress history
...
9. Phase 9: Completed August 19, 2026. Rewrote the Phase 6 pathway query using CTEs, and used window functions (SUM OVER PARTITION BY) to trace how cost accumulates across each patient's treatment timeline. Tracing individual patients surfaced an error in the Phase 6 explanation: the cost gap is not caused by targeted drug pricing. Medications are 1-3% of diagnosis-scoped cost across all pathways, and the hormone/targeted group spends less on drugs ($762 avg) than the chemo group ($1,238 avg) while costing roughly $14K more overall. The actual driver is radiation and surgery. The hormone/targeted pathway substitutes a radiation course (102 external beam events, plus 68 radiation therapy care events) for chemotherapy; the chemo group has zero radiation events. The finding held, the mechanism was wrong. Caveat: n=5 per group, so the radiation contrast is the solid part while the lumpectomy frequency difference (4 vs 2) is far less reliable at this sample size.
...
Using synthetic patient data generated by Synthea, structured through PostgreSQL, and analyzed step by step in SQL, the project confirmed the first question directly: this cohort's average healthcare cost runs nearly double that of the general population. The second question produced a less expected answer. Rather than chemotherapy driving the highest cost, patients on targeted hormone therapy without chemotherapy showed the highest average cost per diagnosis. Digging into the event-level data explained why, and corrected an assumption I had made earlier. Medications account for only 1-3% of diagnosis-scoped cost across every pathway, and the hormone/targeted group actually spends less on drugs than the chemo group. The cost difference comes from procedures: that group's treatment is built around surgery and a radiation course, while the chemo group received no radiation at all.
